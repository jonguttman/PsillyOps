...
├── LabelPreviewButton.tsx    # Preview modal with QR controls
├── QRBoxOverlay.tsx          # Visual drag/resize overlay for QR positioning
...
| **Auto-Generated Placeholders** | SVGs without `<g id="qr-placeholder">` get fallback placeholder |
| **Label Size Override** | Temporarily change label dimensions for preview (non-destructive) |

### Enhanced Label Preview (Phase 1)

The preview system provides advanced controls while maintaining non-destructive rendering:

#### Unified Preview Metadata

Preview APIs return both SVG content and metadata in a single response:

```typescript
interface LabelPreviewResult {
  svg: string;
  meta: {
    widthIn: number;           // Current label width in inches
    heightIn: number;          // Current label height in inches
    hasPlaceholder: boolean;   // Whether original SVG had placeholder
    placeholderBox: { x, y, width, height } | null;  // Placeholder bounds
    isAutoGenerated: boolean;  // Whether placeholder was auto-generated
  };
}
```
...
**Design Principle:** Label size override is treated as a layout instruction applied at render time. The original SVG remains unchanged in storage.

```mermaid
flowchart LR
    A[Original SVG] --> B{Size Override?}
    B -->|No| C[Use Native Size]
    B -->|Yes| D[Apply viewBox Transform]
    C --> E[Render with QR]
    D --> E
    E --> F[Return SVG + Metadata]
```

**Implementation:**
- `applyLabelSizeOverride()` modifies SVG `width`/`height` attributes
- ViewBox is added/updated to preserve content scaling
- Original file on disk is never modified
- Sheet tiling recalculates based on overridden dimensions

#### QR Position Controls

The preview modal provides:
- **Scale slider**: 10% to 150% of placeholder size
- **Position nudge**: Directional pad for fine-tuning offset
- **Reset to Default**: Returns QR to placeholder position (or auto-generated default)
- **Save Position**: Persists settings to `LabelTemplateVersion` (qrScale, qrOffsetX, qrOffsetY)

#### Service Layer Functions

```typescript
// Get label metadata without full render
getLabelMetadata(versionId: string): Promise<LabelMetadata>

// Render with metadata (used by preview APIs)
renderLabelPreviewWithMeta(
  ...
  overrides?: {
    ...
    qrOffsetX?: number;
    qrOffsetY?: number;
    labelWidthIn?: number;   // Size override (preview only)
    labelHeightIn?: number;  // Size override (preview only)
  }
): Promise<LabelPreviewResult>

// Non-destructive size transformation
applyLabelSizeOverride(
  svg: string,
...
### Smart QR Placement (Phase 2.5 + 2.6)

The preview system includes intelligent QR positioning helpers:

#### QR Box Position Model

```typescript
interface QrBoxPosition {
  xIn: number;     // X position from left edge (inches)
  yIn: number;     // Y position from top edge (inches)
  widthIn: number; // QR width (inches)
  heightIn: number; // QR height (inches)
}
```
...
suggestQrPlacement({ labelWidthIn, labelHeightIn, minQrSizeIn?, marginIn? }): QrBoxPosition

// Calculate maximum safe QR size (centered)
calculateMaxQrSize({ labelWidthIn, labelHeightIn, marginIn? }): QrBoxPosition

// Find candidate regions for QR placement (heuristic-based)
findLargestEmptyRegion({ labelWidthIn, labelHeightIn, qrSizeIn?, marginIn? }): SuggestedRegion[]

// Convert between absolute position and offset/scale
...
1. **Preview-Only**: All smart placement is preview-layer; never modifies stored SVGs
2. **Non-Destructive**: Offset/scale derived from absolute position
3. **User Consent**: Never auto-saves suggestions; user must click Save
4. **Heuristic-Based**: Fast zone detection without geometric collision
5. **Backward Compatible**: Existing templates and saved positions unchanged
...
| `/api/labels/preview` | POST | Single-label preview with metadata (JSON) |
| `/api/labels/preview-sheet` | POST | Sheet preview with metadata (JSON) |
...
```json
{
  ...
  "qrScale": "number (0.1-1.5, optional)",
  "qrOffsetX": "number (optional)",
  "qrOffsetY": "number (optional)",
...
{
  "svg": "<svg>...</svg>",
  "sheetMeta": { "columns": 3, "rows": 4, "perSheet": 12, "rotationUsed": false, "totalSheets": 1 },
  "labelMeta": { "widthIn": 2.5, "heightIn": 3.0, "hasPlaceholder": true, "placeholderBox": {...}, "isAutoGenerated": false }
}
```
...
## 📋 Phase 4: Operational History, Accountability & Supply Flow

### Overview

Phase 4 provides complete visibility into operational history and supply chain management by answering three critical questions:

1. **What happened?** - Activity Log
2. **Why did it happen?** - Context + QR + AI correlation
3. **What do we need to buy?** - Purchase Orders

### Module Architecture

```
app/(ops)/activity/
├── page.tsx                    # Activity Log page (server)
└── ActivityLogClient.tsx       # Main client component with filters

app/(ops)/purchase-orders/
├── page.tsx                    # PO list (server)
├── PurchaseOrdersClient.tsx    # PO list (client)
├── [id]/
│   ├── page.tsx                # PO detail (server)
│   └── PurchaseOrderDetailClient.tsx  # PO detail (client)
└── new/
    ├── page.tsx                # Create PO (server)
    └── NewPurchaseOrderClient.tsx     # Create PO (client)

components/activity/
├── ActivityLogFilters.tsx      # Filter toolbar
└── ActivityLogRow.tsx          # Expandable activity row

components/dashboard/
└── SupplyWatchCard.tsx         # Supply awareness card
...
app/api/activity/
└── route.ts                    # GET with enhanced filters (dates, tags)
```

### Phase 4A - Activity Log

#### Key Features

- **Unified Activity Timeline**: Single table showing Time, Actor, Action, Entity, Quick links
- **Smart Filters**: Entity type, User, Time range, Action category
- **Expandable Rows**: Click to show detailed diff, related entities, AI context
- **Entity Deep Links**: Direct navigation to relevant pages
- **Pagination**: Load more with infinite scroll
- **Lightweight API**: No full joins, lazy resolution for performance

#### Filter Categories

- **Entity Types**: QR, Product, Batch, Inventory, PO, Order, Material, Vendor, Label, System
- **Actors**: User dropdown + System option
- **Time Ranges**: Last 24h (default), 7 days, 30 days, All time
- **Action Categories**: Created, Status Changes, Quantity Changes, Movements, QR Scans, AI Commands, Shortages

#### Entity Deep Links

Each activity row links to the relevant entity:
...
- Order → `/orders/[id]`
- QRToken → `/qr/[tokenId]` (matches Phase 3 implementation)
- Material → `/materials/[id]`
- Vendor → `/vendors/[id]`

### Phase 4B - Purchase Orders
...
erDiagram
    ...
    PurchaseOrderLineItem }o--|| RawMaterial : "specifies"
...
#### Receiving Workflow (Append-Only)

**Design Principle**: Treat each receive as an event, not a mutation overwrite. This makes audits and mistakes survivable.

**Invariants:**
- Received qty ≤ ordered qty (enforced)
- Cannot receive on CANCELLED or DRAFT status
- Receiving is append-only (each receipt logged)

**Process:**
1. Validate: PO status is SENT or PARTIALLY_RECEIVED
2. Validate: received qty ≤ (ordered - already received) per line
3. Append receipt event to ActivityLog with full details
4. Increment `quantityReceived` on PO line items
5. Create inventory records via `receiveMaterials()`
6. Update `currentStockQty` on RawMaterial
7. If all lines fully received → status = RECEIVED, else PARTIALLY_RECEIVED

**Receipt Tracking**: Each receive call logs a detailed receipt record in ActivityLog with:
- Line item IDs and quantities received
- Lot numbers / expiry dates if provided
...
```typescript
// PO creation
createPurchaseOrder(input: CreatePurchaseOrderInput, userId: string): Promise<string>

// Update DRAFT only
updatePurchaseOrder(id: string, input: UpdatePurchaseOrderInput, userId: string): Promise<void>

// Submit to vendor
submitPurchaseOrder(id: string, userId: string): Promise<void>

// Receive items (append-only)
...
// Cancel PO
cancelPurchaseOrder(id: string, reason: string, userId: string): Promise<void>

// Queries
getPurchaseOrder(id: string)
listPurchaseOrders(filters: PurchaseOrderFilters)
getPurchaseOrdersCount(filters)

// Helpers
calculatePOTotal(lineItems): number
getPOStatusColor(status): string
```
...
| Route | Method | Auth | Description |
|-------|--------|------|-------------|
| `/api/purchase-orders` | GET | OPS+ | List POs with filters |
| `/api/purchase-orders` | POST | ADMIN/WAREHOUSE | Create PO |
| `/api/purchase-orders/[id]` | GET | OPS+ | Get detail |
| `/api/purchase-orders/[id]` | PATCH | ADMIN/WAREHOUSE | Update DRAFT |
| `/api/purchase-orders/[id]` | DELETE | ADMIN | Cancel PO |
| `/api/purchase-orders/[id]/submit` | POST | ADMIN/WAREHOUSE | Submit to vendor |
| `/api/purchase-orders/[id]/receive` | POST | ADMIN/WAREHOUSE | Receive items |
...
Features:
- Table with PO #, Vendor, Status, Items, Value, Expected delivery, Created
...
Sections:
1. **Header**: PO#, Status badge, Action buttons (Submit, Receive)
2. **Summary Card**: Vendor info, expected delivery, notes
3. **Line Items Table**: Material, SKU, Ordered, Received, Unit Cost, Total
4. **Receiving Panel**: "Mark Received" with quantity inputs per line
5. **Activity Timeline**: Embedded activity feed for this PO
...
- Notes
- Dynamic line items: Material + Quantity + Unit Cost
- Add/remove line items

### Phase 4.3 - Dashboard Supply Awareness (Revised)

#### Supply Watch Card

Dashboard component (`SupplyWatchCard.tsx`) showing read-only operational risk and accountability:

- **Inventory Risk**
  - 🔴 Materials below reorder point (count + deep links)
  - 🟡 Materials with recent manual adjustments (last 48h) (count + deep links)
- **Purchasing State**
  - 📦 Open purchase orders (DRAFT, SENT, PARTIALLY_RECEIVED) with link to `/purchase-orders`
  - ⏱ Days since last PO received

Displays:
- Alert icons for issues
- Checkmark icons for good status
- Links to relevant pages (Materials, Purchase Orders)

**Data sources (service-aligned):**
- `inventoryService.getLowStockMaterials()`
- `inventoryAdjustmentService.getRecentAdjustments()`
- `purchaseOrderService.listPurchaseOrders()`
- Activity Log (for “last received” correlation where available)

#### Dashboard Layout Update

Placement order (intentional):
1. AlertsPanel
2. SupplyWatchCard
3. RecentQRScans
4. ActivityFeed

#### REQUIRED PRE-STEP (after Prisma reset/migrations)

After any Prisma reset or migration (Phase 4.2+), run:

`npx prisma db seed`

This restores materials, inventory rows, reorder points, and example on-hand quantities required for UI validation.

#### Inventory Adjustment Source of Truth

Inventory corrections and accountability are centralized in `inventoryAdjustmentService` (always writes an `InventoryAdjustment` record, updates inventory quantities transactionally, and logs to Activity Log for narrative visibility).

### Phase 4D - Menu Reorganization

#### New Sidebar Structure

```
OPS (expanded by default)
├─ Dashboard
├─ Scan QR
├─ Products
├─ Materials
...
├─ Production
├─ Orders
├─ Purchasing  (NEW - menu label for /purchase-orders)

LABELS (collapsed)
├─ Templates

QR (collapsed - ADMIN only)
├─ Redirect Rules

SYSTEM (collapsed)
├─ Activity  (MOVED from Intelligence)
├─ Strains
├─ Vendors
├─ Help
```
...
**Naming Convention:**
- Menu label: `Purchasing` (how people talk)
- Page title: `Purchase Orders` (what they see)
- Entity name: `PurchaseOrder` (how systems model)

### Activity Log Enhancements

#### API Improvements

Enhanced `/api/activity` with:
- `startDate` and `endDate` query params for date range filtering
- `tags` param for action category filtering
- `userId=system` for filtering system actions (null userId)
- Lightweight response - no full joins, lazy resolution

#### Service Layer Updates

`loggingService.ts`:
- Updated `ActivityFeedFilters` interface to support `userId: string | null`
- Updated `getActivityFeed()` to handle null userId (system actions)
- Tags filter using SQLite-compatible `string_contains`

### Key Design Decisions

1. **QR Token Links**: Standardized on `/qr/[tokenId]` for consistency with Phase 3
2. **Activity API**: Lightweight - no full joins, UI resolves display names lazily
3. **Receiving Workflow**: Append-only with validation - each receipt is logged as an event
4. **Menu Naming**: Clear separation between menu labels (user-facing) and entity names (technical)

### Explicitly Deferred

To keep Phase 4 shippable, the following are NOT included:

- ❌ Vendor performance analytics
- ❌ Automatic PO generation (MRP → PO automation)
- ❌ Cost rollups / margin analysis
- ❌ Alerts beyond "below reorder point"
- ❌ Full entity metadata joins in activity API
- ❌ AI PO commands (marked as optional/conservative)
...
**Inventory Service:**
- `receivePurchaseOrderItems()` calls `receiveMaterials()` to create inventory
- Updates `RawMaterial.currentStockQty`
- Creates `InventoryItem` records

**Logging Service:**
- All PO actions logged to `ActivityLog` with entity type `PURCHASE_ORDER`
- Receipt events logged with full details (line items, quantities, lot numbers)

**Dashboard:**
- Queries `PurchaseOrder.count()` for open POs
- Queries last received PO for days calculation
- Queries materials below reorder point for alerts
...
## Addendum (v0.14.2): AI Assist — Navigation, Lookup, and Confirmed Create

This addendum documents the evolution of the AI Command system from “interpret + execute” into a safer, operator-trust-first design:

- **Phase 1 (Navigate + Prefill)**: AI routes to the right UI and optionally pre-fills form fields.
- **Phase 1.5 (Read-only navigation + lookup)**: AI can open screens and best-match existing entities (no writes).
- **Phase 2 (Confirmed creation)**: AI can propose creation of Strains/Materials, but the write requires an explicit click confirmation.

### Design Principles (Enforced)

- **No silent writes**: AI never creates records in the background.
- **Explicit confirmation**: Write actions require a UI button click (**Create**).
- **Auditability**: Every proposal, navigation, and confirmed creation is logged.
- **Duplicate safety**: If an entity already exists, AI navigates to it instead of creating a duplicate.

---

## New/Updated Routes (Ops UI)

- **Strains**
  - `GET /strains` — list + management view (ADMIN)
  - `GET /strains/new` — canonical create form (ADMIN)
  - `GET /strains/[id]` — strain detail (ADMIN)

### Backward Compatibility

Older links like `/strains#add-strain` are redirected client-side to `/strains/new`.

---

## AI Command Endpoint: New Response Shapes

Endpoint: `POST /api/ai/command`

### 1) Navigation response (read-only)
...
```json
{
  ...
  "prefill": { "name": "Capsules", "categoryHint": "PACKAGING" }
}
```

### 2) Proposed action (no write yet)
...
```json
{
  ...
  "type": "PROPOSE_CREATE",
  "entity": "strain",
  "prefill": { "name": "Penis Envy", "shortCode": "PE" },
  "confirmationText": "I can create a new strain named Penis Envy. Would you like me to create it now?"
}
```

### 3) Confirmed create call path (write happens here)

The client submits a second request only after the user clicks **Create**:
...
The API then returns a `NAVIGATION` response to the created (or existing) entity.

---

## Activity Logging (AI)

AI-related activity is intentionally lightweight and framed as assistive:

- `ai_navigate_add_entity` — **AI prepared add form**
- `ai_navigate_entity` — **AI prepared navigation**
- `ai_propose_create_entity` — **AI proposed create** (no write)
- `ai_create_entity_confirmed` — **AI created entity (confirmed)** with snapshot in `details.after`

### Data Model Notes

- No schema changes were required for these phases.
- All AI audit records are stored using the existing `ActivityLog` + `AICommandLog` tables.
...
```mermaid
flowchart LR
  U[User] --> UI[AiCommandBar / DashboardAiInput]
  UI --> API[POST /api/ai/command]
  API --> SVC[aiCommandService: interpret + resolve]
  SVC --> DB[(Prisma)]

  API -->|NAVIGATION| UI
  API -->|PROPOSE_CREATE| UI

  UI -->|User clicks Create| API
  API -->|Create record| DB

  API --> LOG[loggingService.logAction]
  LOG --> ACT[(ActivityLog)]
```

---

## Files Added / Modified (v0.14.2)
