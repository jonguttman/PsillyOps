// API Route: Production Run Step - Stop
// STRICT LAYERING: Validate → Call Service → Return JSON

import { NextRequest } from 'next/server';
import { auth } from '@/lib/auth/auth';
import { handleApiError } from '@/lib/utils/errors';
import { stopStep } from '@/lib/services/productionRunService';
...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ stepId: string }> }) {
  try {
    ...
    const result = await stopStep(stepId, session.user.id, session.user.role);
